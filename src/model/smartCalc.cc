...
LoanResults SmartCalc::calcCredit(LoanInfo input) {
  LoanResults results = {0};
  double monthlyRate = input.annualRate / 1200;

  if (input.type == LoanType::ANNUITY) {
  ...
  } else if (input.type == LoanType::DIFFERENTIATED) {
...
DepositResults SmartCalc::calcDeposit(DepositInfo info) {
